  ...
  useEffect(() => {
    ...
    if (chapter) {
      form.setFieldsValue({
        ...
        video_storage_key: chapter.video_storage_key ?? undefined,
      ...
      <Form form={form} layout="vertical">
        ...
        <Form.Item name="video_source_type" label="视频来源" rules={[{ required: true }]}>
          <Select options={[
            ...
            { value: 'private_object', label: '私有对象存储' },
          ]} />
        </Form.Item>
        {sourceType === 'external_url' ? (
          ...
          <Form.Item name="video_storage_key" label="私有视频对象键" rules={[{ required: true, message: '请输入对象键' }]}>
...
export default function CourseManagementPage() {
  ...
  const uploadAsset = async (file: UploadFile) => {
    if (!activeCourse) return false
    const form = new FormData()
    form.append('file', file as unknown as File)
    form.append('title', file.name)
    form.append('asset_type', file.type || 'file')
  ...
  const chapterColumns: ColumnsType<CourseChapter> = [
    ...
    { title: '视频来源', dataIndex: 'video_source_type', width: 110, render: value => value === 'external_url' ? '外部 URL' : '私有对象' },
            ...
            <Tabs
              items={[
                ...
                },
                {
                  ...
                  children: (
                    <>
                      <Upload maxCount={1} showUploadList={false} beforeUpload={uploadAsset} customRequest={() => undefined}>
